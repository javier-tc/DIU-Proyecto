...
function App() {
	...
	return (
		<div className="semana-container">
			...
			>
				{mostrarPrecios ? 'Ocultar Presupuesto' : 'Mostrar Presupuesto'}
			</button>
			{mostrarPrecios && (
				<ul className="precios-ul">
					...
					{Object.keys(ingredientesSeleccionados).map((ingrediente, index) => (
						ingredientesSeleccionados[ingrediente] && (
							<li className='precios-li' key={index}>
						...
						{Object.keys(comidasPorDia).map((tipoComida, index) => (
							<div className="comida-container" key={index}>
								...
								<select
									...
									onChange={(e) => {
										handleAgregarComida(dia, tipoComida, e.target.value);
										obtenerIngredientes();
									}}
								>
